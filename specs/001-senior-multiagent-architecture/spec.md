...
1. **Given** a completed `spec.md`, **When** the Orchestrator triggers the QA-Spec agent, **Then** a `qa-gates/spec-gate.json` is produced within 60 seconds with valid status and findings.
...
2. **Given** a spec with `[NEEDS CLARIFICATION]` marker, **When** QA-Spec agent runs, **Then** gate status is FAIL with finding type `AMBIGUOUS_REQUIREMENT`.
...
- **FR-006**: System MUST persist every gate report as a JSON file in `.specify/specs/NNN/qa-gates/<phase>-gate.json`.
...
- Mobile/browser-specific E2E configurations are out of scope for v1; cross-browser matrix is a future enhancement.
