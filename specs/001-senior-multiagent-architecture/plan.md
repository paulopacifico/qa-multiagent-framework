...
│  Persists gate report under specs/<feature-id>/qa-gates/         │
└────────────────────┬─────────────────────────────────────────────┘
...
- **Gate Persistence**: JSON files in `specs/NNN-feature-name/qa-gates/`
...
receive request
  ...
  → persist to specs/<feature-id>/qa-gates/<phase>-gate.json
...
### 6. Gate Persister (`src/orchestrator/gate-persister.ts`)

**Responsibility**: Validate and persist gate reports to the feature-scoped `qa-gates/` directory.

**Checklist**:
- [ ] Build the output path from `feature_id`
- [ ] Create the feature `qa-gates/` directory when absent
- [ ] Reject malformed gate reports before writing JSON

---

### 7. Gate Enforcer (`src/orchestrator/gate-enforcer.ts`)

**Responsibility**: Convert gate status into progression decisions.

**Checklist**:
- [ ] Block FAIL without override
- [ ] Block WARN unless `accept_warn` and `acceptance_reason` are present
- [ ] Allow PASS without additional approval

---

### 8. GateReport Interface (`src/types/gate-report.ts`)

**Responsibility**: Provide the shared typed contract for gate reports and decisions.

**Checklist**:
- [ ] Define phase, agent, status, findings, recommendation, and timestamp fields
- [ ] Keep agent names and gate statuses constrained to known values

---

### 9. GitHub Actions Workflow (`.github/workflows/qa-gates.yml`)

**Responsibility**: Run typecheck, lint, unit tests, E2E tests, and QA gates in the PR pipeline.

**Checklist**:
- [ ] Execute QA gates after Playwright has generated Allure results
- [ ] Upload Allure results and feature-scoped QA gate reports as artifacts
...
|-------------|-----------|
| FR-001 | Orchestrator Agent — phase routing and dispatch |
| FR-002 | QA-Spec Sub-Agent — spec validation checklist |
| FR-003 | QA-Plan Sub-Agent — plan and data model validation |
| FR-004 | QA-Code Sub-Agent — POM compliance and selector hygiene |
| FR-005 | QA-CI Sub-Agent — GitHub Actions run result validation |
| FR-006 | Gate Persister — writes gate report JSON per phase |
| FR-007 | Gate Enforcer — throws GateBlockedError on FAIL status |
| FR-008 | Gate Enforcer — requires accept_warn flag and acceptance_reason for WARN |
| FR-009 | GateReport interface — phase, agent, status, findings, recommendation, timestamp |
| FR-010 | GitHub Actions workflow — qa-gates.yml runs unit and E2E gates on every PR |
