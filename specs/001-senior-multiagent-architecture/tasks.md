...
- [ ] 4.1 Implement gate report persistence
  - `src/orchestrator/gate-persister.ts`: writes gate report to `.specify/specs/<feature_id>/qa-gates/<phase>-gate.json`
...
- [ ] 5.1 Create QA Gates GitHub Actions workflow
  - `.github/workflows/qa-gates.yml`: 5-job pipeline (qa-spec → qa-plan → qa-code → playwright → qa-ci)
...
- [ ] 6.1 Run full QA gate pipeline locally
  ...
  - All 4 gates must PASS; gate reports persisted to `qa-gates/`
  - **Depends on**: 4.1, 5.2
